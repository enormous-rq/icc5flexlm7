/*****************************************************************************r
	    ...
	    COPYRIGHT (c) 1988, 1999 by Globetrotter Software Inc.
 ...
 *****************************************************************************/
/*
 *	Module: $Id: lmclient.h,v 1.28.4.6 1999/11/24 19:11:17 sandras Exp $
 *
...
#define LM_CO_METER_BORROW	5	/* used by lc_borrow() */
...
/* used by borrowing on Windows  */
typedef struct _lm_meters {
	struct _lm_meters * next;
	char device[100]; 
	char vendor[MAX_DAEMON_NAME + 1];
	void *meter_handle;
	int meter_lock_val; /* internal use */
}  LM_METERS ; 
...
#define LM_MAX_BEH_VER 4
			  ...
			  } VENDORCODE6,  FAR *VENDORCODE_PTR;
...
#define LM_BEHAVIOR_CURRENT	"07.0"
...
#define LM_TYPE_METER_BORROW   0x40 
			...
			LM_CHAR_PTR lc_meter_borrow_info;/*(not authenticated)*/
...
#define LM_CONF_DECIMAL_FMT 	LM_CONF_DECIMAL_FMT_V6
			...
			char *date_sort_str;	/* for internal use */
 			...
 			char *meter_borrow_device; 
 			int meter_borrow_counter;
/*
 *			Links
 */
...
#define LM_BORROW_METER 1
lm_extern int API_ENTRY lc_borrow lm_args((LM_HANDLE_PTR , int , 
			LM_CHAR_PTR_PTR, 
			LM_CHAR_PTR, LM_CHAR_PTR, VENDORCODE_PTR, LM_CHAR_PTR, 
			LM_CHAR_PTR, LM_CHAR_PTR, long, long));
lm_extern int API_ENTRY lc_borrow_dump 	lm_args((LM_HANDLE *, int, 
				char *, int *, int *, unsigned int *, 
						long *));
lm_extern int lc_borrow_find_meter		lm_args (( LM_HANDLE *,
						LM_METERS **));
lm_extern int API_ENTRY lc_borrow_free_counter	lm_args((LM_HANDLE *, int,
						char **, int));
lm_extern void lc_borrow_free_meter		lm_args((LM_HANDLE *, 
							LM_METERS *));
lm_extern int API_ENTRY lc_borrow_get_counter	lm_args(( LM_HANDLE *, int,
					char **, int, int *, unsigned int *));
lm_extern int API_ENTRY lc_borrow_increment	lm_args((LM_HANDLE *, int,
					char **, int, int, char *));
lm_extern int API_ENTRY lc_borrow_make_counter	lm_args(( LM_HANDLE *, int,
					char **, int, int, unsigned int *));
lm_extern int API_ENTRY lc_borrow_update_key	lm_args((LM_HANDLE *, int,
						char *, int, int,
						unsigned int, char **));
lm_extern int API_ENTRY lc_borrow_init	lm_args((LM_HANDLE *, int, char **));
lm_extern int API_ENTRY lc_borrow_sn	lm_args(( LM_HANDLE *, int, char **, 
						char **));
lm_extern int API_ENTRY lc_borrow_vendor lm_args(( LM_HANDLE *, int ,
						char **, char **));
...
lm_extern int API_ENTRY lc_new_job lm_args((LM_HANDLE_PTR, LM_CHAR_PTR, 
...
lm_extern int API_ENTRY lc_unborrow	lm_args((LM_HANDLE *, int , char **, 
					char *, char *, int, long, long));
